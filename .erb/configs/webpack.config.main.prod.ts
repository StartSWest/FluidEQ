...
const configuration: webpack.Configuration = {
  ...
  plugins: [
    ...
    new webpack.EnvironmentPlugin({
      ...
      FLUIDEQ_UPDATE_URL: '',
      // The GitHub App the forum signs in through: its id only. Main only, and
      // deliberately not in `public-env`: nothing in the window needs it.
      // Its client secret is never compiled in — the server's `github-token`
      // function holds it (`src/main/forum/forumConfig.ts`). Empty means the
      // forum is read-only, which is right for a fork.
