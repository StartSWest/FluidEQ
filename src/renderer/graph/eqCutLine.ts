...
import { IEqCuts, MIN_GAIN } from 'common/constants';
 ...
 * plot cannot draw. So a cut takes the curve down to the bottom of the fixed
 * axis (`gainScale`) and no lower, unless the curve without it was already
 * lower.
 */
const withEqCuts = (
  ...
  return points.map((point, index) => {
    ...
    const floor = Math.min(MIN_GAIN, point.y);
