...
import { LIGHTING_TICKS_PER_SECOND as AUDIO_CLOCK_TICKS_PER_SECOND } from '../lighting/lightingSceneMessages';
...
/**
 * The most audio a measurement somebody is waiting on may listen to, counted
 * on the audio clock.
 *
 * Silence ends a measurement and so does hearing enough, but a record can be
 * neither: quieter than a frame the measurement will learn from yet louder
 * than silence, or missing a whole end of the spectrum from start to finish.
 * Those frames buy no listened time and do not count as silence either, so
 * nothing else would ever end the run.
 *
 * It was a two-minute wall-clock timer, which a minimised window throttles to
 * one a second and then to one a minute: it fired late or not at all exactly
 * where a measurement is usually left running. Counted in audio instead, it is
 * two minutes of sound that went past the capture, wherever the window is.
 */
export const MEASUREMENT_AUDIO_LIMIT_MS = 120000;
