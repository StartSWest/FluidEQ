 ...
 */
/**
 * Auto balance: measure the record as it was made — the sound before FluidEQ
 * processes it — then flatten the peaks and dips while leaving the music's own
 * spectral tilt alone.
 *
 * Everything in this file is pure — plain numbers and typed arrays, no Web
 * Audio, no wall clock. The measurement session owns the audio plumbing and
 * feeds frames in; every decision about whether we have heard enough is made
 * here, so it can be driven by synthetic frames in a test.
 */
