...
/**
 * Pack measured labels into three stable rows, then make a backwards pass so
 * the final words remain visible at the right edge. When one row can fit its
 * labels, this guarantees they neither overlap nor leave the plot bounds.
 */
export const layoutKaraokeMakerLyricLabels = (
  labels: readonly IKaraokeMakerLyricLabel[],
  plotLeft: number,
  plotRight: number,
  laneCount = KARAOKE_MAKER_LYRIC_LANE_COUNT,
  ...
  const safeLaneCount = Math.max(1, Math.round(laneCount));
  const laneRightEdges = new Array<number>(safeLaneCount).fill(plotLeft - gap);
  const placed = labels.map((label) => {
    const lane = karaokeMakerLyricLane(
      laneRightEdges,
      label.naturalLeft,
      label.preferredLane,
      gap,
    );
    const left = Math.max(label.naturalLeft, laneRightEdges[lane] + gap);
    laneRightEdges[lane] = left + label.width;
    return { ...label, lane, left };
  });

  for (let lane = 0; lane < safeLaneCount; lane += 1) {
    const row = placed.filter((label) => label.lane === lane);
    let nextLeft = plotRight + gap;
    for (let index = row.length - 1; index >= 0; index -= 1) {
      const label = row[index];
      label.left = Math.min(
        label.left,
        plotRight - label.width,
        nextLeft - gap - label.width,
      );
      nextLeft = label.left;
    }
    let previousRight = plotLeft - gap;
    row.forEach((label) => {
      label.left = Math.max(plotLeft, label.left, previousRight + gap);
      previousRight = label.left + label.width;
    });
  }
  return placed;
};
