...
describe('the handshake', () => {
  it('is waited for with no deadline, and a host that closes its output without one is let go and counted as a crash', async () => {
    const host = supervisor();
    ...
    first.stdout.end();
    await settle();
    // Made to go, and not judged before its exit arrives: refused here as a
    // failed handshake, a restarted host that died starting ended the
    // supervisor with no retry.
    expect(first.kill).toHaveBeenCalled();
    expect(answer).toBeUndefined();
    first.emit('close', 1, null);
    await settle();
    expect(answer).toBe(false);
    // The exit was a crash, and the budget brings a host back.
    expect(hosts).toHaveLength(2);
    expect(host.getState()).not.toBe('failed');
    expect(jest.getTimerCount()).toBe(0);
  });
