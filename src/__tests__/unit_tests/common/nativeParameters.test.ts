...
describe('the native DSP parameter table', () => {
   ...
   * that merge, and that none of the seventeen ids collided with a twin.
   */
  it('reserves 2000-2199 for the bass stages and burns nothing', () => {
    const bass = NATIVE_DSP_PARAMETERS.filter(
      (p) => p.id >= 2000 && p.id < 2200,
    );
    expect(bass).toHaveLength(17);
    expect(new Set(bass.map((p) => p.id)).size).toBe(17);
    expect(bass.find((p) => p.path === 'bassPunch.mix')?.id).toBe(2109);
