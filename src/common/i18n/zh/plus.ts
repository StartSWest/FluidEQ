const plus = {
  'plus.remove.confirm': '从你的外观中移除？',
  'plus.remove.failed': '无法移除 {name}，请重试。',
  'plus.remove.done': '已从你的外观中移除 {name}。',
  'plus.card.remove': '移除',
  'plus.official.author': 'FluidEQ · 官方',
  'plus.official.included': 'Plus 已包含',
  'plus.official.sample': '{seconds} 秒预览',
  'plus.official.badge': '官方',
  'plus.official.fine':
    'FluidEQ 合集的一部分：在你拥有 Plus 期间可从外观中播放。',
  'plus.visualizers.title': '可视化',
  'plus.visualizers.description':
    'FluidEQ 和 Plus 社区创作的场景，随你的音乐律动',
  'plus.visualizers.blurb': '会员的场景',
  'plus.rail.pin': '固定侧边栏',
  'plus.rail.collapse': '收起侧边栏',

  'plus.gate.cta': '了解 Plus',
  'plus.browse.text':
    '有了 Plus，这些场景会随你的音乐播放，还能添加到你的外观并点赞。',

  'plus.gallery.search': '搜索场景或作者',
  'plus.gallery.sort': '排序',
  'plus.gallery.sort.liked': '最多赞',
  'plus.gallery.sort.week': '本周',
  'plus.gallery.sort.new': '最新',
  'plus.gallery.categories': '分类',
  'plus.gallery.all': '全部',
  'plus.gallery.mine': '你的场景',
  'plus.gallery.more': '显示更多',
  'plus.gallery.loading': '正在加载场景…',
  'plus.gallery.empty.title': '这里还没有场景',
  'plus.gallery.empty.body': '在工作室里创作一个并发布，它将是第一个。',
  'plus.gallery.empty.search': '没有与“{query}”匹配的结果。',
  'plus.gallery.empty.category': '这个分类里还没有场景。',
  'plus.gallery.empty.openStudio': '打开工作室',
  'plus.gallery.error.offline': '图库需要网络连接。请联网后重试。',
  'plus.gallery.error.signedOut': '请重新登录以查看图库。',
  'plus.gallery.error.server': '无法加载图库。请重试。',
  'plus.gallery.moreError': '无法加载更多场景。请重试。',
  'plus.gallery.retry': '重试',

  'plus.category.nature': '自然',
  'plus.category.cities': '城市',
  'plus.category.space': '太空',
  'plus.category.water': '水',
  'plus.category.fire-light': '火与光',
  'plus.category.retro-games': '复古游戏',
  'plus.category.animals': '动物',
  'plus.category.abstract': '抽象',
  'plus.category.worlds-3d': '3D 世界',

  'plus.card.by': '作者：{name}',
  'plus.card.byYou': '你的作品',
  'plus.card.anonymous': '一位 Plus 会员',
  'plus.card.adds': '{count} 人添加',
  'plus.card.add': '添加',
  'plus.card.added': '已添加',
  'plus.card.updateTo': '更新到 v{version}',
  'plus.version.new': '新版本',
  'plus.version.line': 'v{version} · {when}',
  'plus.version.youHave': '你的是 v{version}',
  'plus.version.title': '版本',
  'plus.version.pageLine': '版本 {version} · {date}',
  'plus.version.whatsNew': '版本 {version} 的新内容',
  'plus.version.earlier': '更早的版本（{count}）',
  'plus.version.short': 'v{version}',
  'plus.version.noNote': '没有说明',
  'plus.card.open': '打开 {name}',

  'plus.like.label': '为 {name} 点赞（{count} 个赞）',
  'plus.like.own': '你的场景获得了 {count} 个赞',
  'plus.like.plusOnly': '{count} 个赞。给场景点赞是 Plus 的功能。',
  'plus.like.offline': '点赞需要网络连接。请联网后重试。',

  'plus.add.done': '{name} 已添加到你的外观。',
  'plus.add.unavailable': '暂时无法下载 {name}。请重试。',
  'plus.add.blocked': '{name} 已被下架。',
  'plus.add.changed': '{name} 未通过 FluidEQ 的检查，因此没有添加。',
  'plus.add.failed': '无法添加 {name}。请重试。',

  'plus.scene.back': '返回',
  'plus.scene.previous': '上一个场景：{name}',
  'plus.scene.next': '下一个场景：{name}',
  'plus.scene.loading': '正在加载场景…',
  'plus.scene.playing': '随你的音乐实时播放',
  'plus.scene.plusPlays': '有了 Plus，这个场景会在这里随你的音乐播放',
  'plus.scene.taste': '预览 · {seconds} 秒',
  'plus.scene.keepWatching':
    '开通 Plus，继续随你的音乐观看，并把它添加到你的外观。',
  'plus.scene.unavailable': '暂时无法下载这个场景。',
  'plus.scene.blocked': '这个场景已被下架。',
  'plus.scene.changed': '这个场景未通过 FluidEQ 的检查，因此不会播放。',
  'plus.scene.heavy': '这个场景对这台电脑来说太吃力了。',
  'plus.scene.broken': '这个场景无法在这台电脑上播放。',
  'plus.scene.cannotDraw': '这台电脑暂时无法绘制场景。',
  'plus.scene.likes': '赞',
  'plus.scene.week': '本周',
  'plus.scene.adds': '添加',
  'plus.scene.add': '添加到我的外观',
  'plus.scene.update': '更新我的副本',
  'plus.scene.getPlus': '开通 Plus 即可添加',
  'plus.inspect.open': '在工作室中打开',
  'plus.inspect.hint':
    '看看它是怎么做的，为你自己的场景汲取灵感。它会作为查看内部的项目打开，而不是用来发布。',
  'plus.inspect.unavailable': '无法下载。请检查网络连接后重试。',
  'plus.inspect.failed': '工作室无法打开它。请重试。',
  'plus.scene.play': '在图表上播放',
  'plus.scene.inLooks': '已在你的外观中，位于“会员创作”',
  'plus.scene.inLooksOwn': '已在你的外观中，位于“你创作的”',
  'plus.scene.inLooksPlus': '已在你的外观中，位于“FluidEQ Plus”',
  'plus.scene.fine':
    '添加的场景在你拥有 Plus 期间可从外观中播放。任何人都不能出售它们。',
  'plus.scene.report': '举报这个场景',
  'plus.scene.reported': '已举报，谢谢。',
  'plus.scene.moreBy': '{name} 的更多作品',
  'plus.scene.moreByYou': '你的更多作品',

  'plus.maker.you': '你',
  'plus.maker.rank': '排名',
  'plus.maker.scenes': '场景',
  'plus.maker.empty': '目前没有已发布的作品。',

  'plus.mine.title': '你的场景',
  'plus.mine.hint': '要发布或更新场景，请在工作室中打开它的项目并点击“发布”。',
  'plus.mine.openStudio': '打开工作室',
  'plus.mine.empty': '你还没有发布任何作品。',
  'plus.mine.published': '发布于 {date}',
  'plus.mine.updated': '更新于 {date}',
  'plus.mine.version': '版本 {version}',
  'plus.mine.blocked': '已被 FluidEQ 下架',
  'plus.mine.unpublish': '取消发布',
  'plus.mine.confirm':
    '要从图库下架并停止新下载吗？已下载的用户会保留自己的副本。',
  'plus.mine.confirmYes': '取消发布',
  'plus.mine.confirmNo': '保留',
  'plus.mine.unpublished': '{name} 已不在图库中。',
  'plus.mine.failed': '无法取消发布。请重试。',

  'plus.gallery.reported': '被举报',
  'plus.gallery.reportedOpen': '{count} 个待审核',
  'plus.moderation.title': '被举报的场景',
  'plus.moderation.hint':
    '会员举报的场景。只有你能看到这个页面，也不会有人知道是谁举报的。下架一个场景会把它从图库和所有添加过它的人的外观中移除。',
  'plus.moderation.list.open': '待审核',
  'plus.moderation.list.takenDown': '已下架',
  'plus.moderation.reason.rights': '未经许可',
  'plus.moderation.reason.flashing': '闪烁',
  'plus.moderation.reason.offensive': '冒犯',
  'plus.moderation.reason.broken': '无法播放',
  'plus.moderation.reports': '举报',
  'plus.moderation.cardTitle': '对这个场景的举报',
  'plus.moderation.lastReport': '最近举报 {date}',
  'plus.moderation.takenDownOn': '下架于 {date}',
  'plus.moderation.makerBanned': '作者已被封禁',
  'plus.moderation.view': '查看',
  'plus.moderation.dismiss': '驳回',
  'plus.moderation.takeDown': '下架',
  'plus.moderation.confirm':
    '要对所有人下架吗？它会从图库和所有添加过它的人的外观中移除。',
  'plus.moderation.confirmNo': '保留',
  'plus.moderation.restore': '恢复',
  'plus.moderation.empty.open': '没有待审核的内容',
  'plus.moderation.empty.openHint': '会员举报场景后，它会在这里等待你处理。',
  'plus.moderation.empty.takenDown': '没有已下架的场景。',
  'plus.moderation.done.takenDown': '{name} 已下架。',
  'plus.moderation.done.dismissed': '已驳回对 {name} 的举报。',
  'plus.moderation.done.restored': '{name} 已回到图库。',
  'plus.moderation.failed': '操作未完成。请重试。',
  'plus.moderation.forbidden': '只有 FluidEQ 管理员可以审核举报。',
  'plus.gallery.gifts': '赠送 Plus',
  'plus.gifts.title': '赠送 Plus',
  'plus.gifts.hint':
    '通过电子邮件地址把 Plus 赠送给任何人。从账户确认该地址起，对方无需付费即可使用 Plus 的全部功能，直到你收回或到达你设定的日期。',
  'plus.gifts.field.email': '电子邮件地址',
  'plus.gifts.field.note': '备注',
  'plus.gifts.field.notePlaceholder': '对方是谁（可选）',
  'plus.gifts.field.until': '结束',
  'plus.gifts.field.untilHint': '结束日期留空则一直有效，直到你收回。',
  'plus.gifts.give': '赠送 Plus',
  'plus.gifts.save': '保存赠送',
  'plus.gifts.empty': '还没有人获赠 Plus',
  'plus.gifts.emptyHint': '你赠送 Plus 的地址会显示在这里。',
  'plus.gifts.status.active': '已拥有 Plus',
  'plus.gifts.status.waiting': '等待账户',
  'plus.gifts.status.ended': '已结束',
  'plus.gifts.untilDate': '至 {date}',
  'plus.gifts.forever': '直到收回',
  'plus.gifts.since': '赠送于 {date}',
  'plus.gifts.edit': '编辑',
  'plus.gifts.takeBack': '收回',
  'plus.gifts.keep': '保留',
  'plus.gifts.confirmTakeBack': '要收回 {email} 的 Plus 吗？',
  'plus.gifts.done.given': '已向 {email} 赠送 Plus',
  'plus.gifts.done.updated': '已保存 {email} 的赠送',
  'plus.gifts.done.takenBack': '已收回 {email} 的 Plus',
  'plus.gifts.error.invalid': '请检查地址，并确认结束日期尚未到来。',
  'plus.gifts.error.forbidden': '只有 FluidEQ 管理员可以赠送 Plus。',
  'plus.gifts.error.failed': '未能完成，请重试。',

  'plus.gallery.accounts': '账户',
  'plus.accounts.title': '账户',
  'plus.admin.title': '管理',
  'plus.admin.blurb': '账户、赠送和举报',
  'plus.admin.description':
    '仅管理员可见：等待你审核的场景、成员举报的场景、所有账户，以及赠送的 Plus',
  'plus.accounts.listHint':
    '最新的在前。点按账户可查看其中的内容，并在账户所有者提出要求时将其永久删除。',
  'plus.accounts.search': '按电子邮件、用户名或名称搜索',
  'plus.accounts.refresh': '刷新',
  'plus.accounts.filter': '显示账户',
  'plus.accounts.filter.all': '全部账户',
  'plus.accounts.filter.plus': '有 Plus',
  'plus.accounts.filter.free': '无 Plus',
  'plus.accounts.admin': '管理员',
  'plus.accounts.loading': '正在加载账户…',
  'plus.accounts.shown': '显示 {shown} / {total}',
  'plus.accounts.empty.none': '还没有账户',
  'plus.accounts.empty.search': '没有与 {query} 匹配的账户',
  'plus.accounts.empty.plus': '这些账户都没有 Plus',
  'plus.accounts.empty.free': '这些账户都有 Plus',
  'plus.accounts.moreError': '无法加载更多账户。请重试。',
  'plus.accounts.error.offline': '账户列表需要网络连接。请联网后重试。',
  'plus.accounts.error.signedOut': '请重新登录以查看账户。',
  'plus.accounts.error.server': '无法加载账户。请重试。',
  'plus.accounts.error.list': '只有 FluidEQ 管理员可以查看账户。',
  'plus.accounts.error.notDeployed': '服务器尚未更新以支持账户列表。',
  'plus.accounts.noName': '未设置名称',
  'plus.accounts.joined': '{date} 加入',
  'plus.accounts.unconfirmed': '地址未确认',
  'plus.accounts.online': '在线',
  'plus.accounts.lastSeen': '上次 {date}',
  'plus.accounts.appVersion': 'FluidEQ {version}',
  'plus.accounts.plan.none': '无 Plus',
  'plus.accounts.plan.paid': 'Plus 至 {date}',
  'plus.accounts.plan.gift': '赠送的 Plus',
  'plus.accounts.plan.giftUntil': '赠送至 {date}',
  'plus.accounts.goes': '一并删除',
  'plus.accounts.fact.published': '已发布的场景',
  'plus.accounts.fact.boardDays': '排行榜天数',
  'plus.accounts.fact.likes': '点过的赞',
  'plus.accounts.fact.adds': '添加的场景',
  'plus.accounts.fact.reports': '发出的举报',
  'plus.accounts.note.admin': '这是管理员账户，管理员账户不能在这里删除。',
  'plus.accounts.note.gone':
    '该账户已删除，但删除没有完成：发布过的文件可能仍在存储中。“完成删除”会移除它们。',
  'plus.accounts.note.deleting':
    '之前对该账户的删除中途停止了。“完成删除”会把它做完。',
  'plus.accounts.note.gift':
    '按照条款，赠送给该地址的 Plus 会保留。如果对方也要求收回，请在“赠送 Plus”中收回。',
  'plus.accounts.give': '赠送 Plus',
  'plus.accounts.giveConfirm': '把 Plus 赠给 {name}？在你收回之前一直有效。',
  'plus.accounts.giving': '正在把 Plus 赠给 {name}…',
  'plus.accounts.giveYes': '赠送 Plus',
  'plus.accounts.giveNo': '暂不',
  'plus.accounts.delete': '删除账户',
  'plus.accounts.finish': '完成删除',
  'plus.accounts.confirm': '要永久删除 {name} 的账户吗？此操作无法撤销。',
  'plus.accounts.working': '正在删除 {name} 的账户及其相关的一切…',
  'plus.accounts.confirmYes': '永久删除',
  'plus.accounts.keep': '保留',
  'plus.accounts.done.title': '账户已删除：{email}',
  'plus.accounts.done.body':
    '该账户及其相关的一切都已删除。移除的已发布文件：{count}。',
  'plus.accounts.error.invalid': '请输入完整地址，例如 name@example.com。',
  'plus.accounts.error.forbidden': '只有 FluidEQ 管理员可以删除账户。',
  'plus.accounts.error.noAccount': '该账户已不存在，也没有剩下需要删除的内容。',
  'plus.accounts.error.unfinished':
    '删除中途停止了。请点击“完成删除”把它做完。',
  'plus.accounts.error.failed':
    '未能完成，请重试：已经开始的删除会接着完成，而不会重新开始。',

  'plus.report.title': '举报 {name}',
  'plus.report.lead': '它有什么问题？只有 FluidEQ 的作者会阅读举报。',
  'plus.report.reason.rights': '未经许可使用了我或他人的作品',
  'plus.report.reason.flashing': '闪烁或让人看着难受',
  'plus.report.reason.offensive': '内容冒犯',
  'plus.report.reason.broken': '无法播放',
  'plus.report.send': '发送举报',
  'plus.report.sending': '正在发送…',
  'plus.report.cancel': '取消',
  'plus.report.failed': '无法发送举报。请重试。',

  'plus.welcome.title': '登录，看看会员们创作了什么',
  'plus.welcome.body':
    '有账户即可浏览图库、观看简短演示。下载并保留场景、完整播放、添加到样式、在工作室里创作自己的场景，以及把你创作的场景带到任何地方，都需要 Plus。',
  'plus.welcome.browse': '浏览会员发布的每一个场景',
  'plus.welcome.play': '用你的音乐播放它们，并添加到你的外观',
  'plus.welcome.studio': '把在工作室里创作的场景带到你的外观、桌面和画廊',
  'plus.welcome.board': '凭借场景获得的点赞登上排行榜',
  'plus.welcome.account.label': '账户',
  'plus.welcome.account.access': '浏览图库并观看简短演示。',
  'plus.welcome.paid.label': '付费Plus会员',
  'plus.welcome.paid.access': '下载并保留场景、完整播放并创建自己的场景。',
  'plus.welcome.fine': '创建账户',
  'plus.welcome.scene.neonHorizon': '霓虹地平线',
  'plus.welcome.scene.nebulaHeart': '星云之心',
  'plus.welcome.scene.hyperdrive': '超空间跃迁',
  'plus.welcome.scene.reefLight': '珊瑚礁之光',
  'plus.welcome.scene.lanternLake': '天灯湖',
  'plus.welcome.scene.prismBloom': '棱镜之花',
  'plus.moderation.delete': '删除',
  'plus.moderation.deleteConfirm':
    '要永久删除吗？它会从图库和所有会员的外观中消失，文件也会被清除，并且再也不能发布。',
  'plus.moderation.deleteForGood': '永久删除',
  'plus.moderation.done.deleted': '{name} 已删除。',
} as const;

export default plus;
