...
const help: Record<keyof typeof en, string> = {
  ...
  'help.room.intro':
    '房间把耳机变成一间听音室。声音的每个声道都成为你头部周围的一只音箱，经由测量得到的头部模型和你亲手塑造的房间反射渲染，于是电影在你面前，游戏在你四周。它需要 FluidEQ Engine 和耳机；在音箱上没有用处。',
  'help.room.steps':
    '打开 DSP，在侧栏选择“房间”并开启。立体声变成你面前的两只音箱；5.1 电影是五只加低音炮；7.1 游戏是整个环绕圈。开关旁的标记会说明当前是哪一种。\n在顶部选一个房间——录音室、客厅、影院、音乐厅等——或自己转动“大小”“墙壁”“距离”，并在环上拖动音箱。播放中的音频流到达不了的音箱会以休眠状态绘制。\n按“适配”，回答五组短暂的听感对比：房间会采用把声音放到你面前的那个头部模型。也可以手动选择小、中、大。\n把喜欢的房间以名称保存；已保存的房间一按即回，且从不更改你的头部设置。',
  'help.room.tip':
    '塑造房间——旋钮、拖动音箱、适配和保存——属于 Plus；房间和头部选择对所有人开放。游戏和电影只会把环绕声道发送到 Windows 认为拥有相应数量音箱的输出：当驱动支持时，输出面板会提供一键切换到 7.1。',
  'help.room.picker':
    '起点房间，像其他处理级的配置一样分组；一经修改即为“自定义”。',
  'help.room.picture':
    '俯视的房间：随吸音而淡去的墙壁、环上的音箱、中央的头部。拖动音箱可移动它。 按下一个音箱可单独设置它：电平、自身距离、以数字表示的角度，以及用于单独试听的静音和独奏。',
  'help.room.dialsName': '大小、墙壁、距离、中置、低音炮',
  'help.room.dials':
