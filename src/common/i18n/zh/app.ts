...
const app: Partial<Dictionary> = {
  'recovery.title': 'FluidEQ 正在恢复',
  'recovery.working':
    '正在停止播放，并使用已保存的设置重新加载窗口。未保存的工作可能会丢失。',
  'recovery.stopped':
    'FluidEQ 无法安全恢复，已停止自动重试。你可以重新加载或退出。未保存的工作可能会丢失。',
  ...
  'app.status.checking': '正在检查音频引擎…',
  'app.status.error': '音频引擎无响应',
  'app.menu.importEq': '导入均衡设置…',
  'app.menu.importConvolution': '导入脉冲响应…',
  'app.menu.restartAudio': '重启 Windows 音频',
  'engine.apo.reconfigure': '重新配置 Equalizer APO',
  'engine.apo.settings': 'Equalizer APO 设置',
  ...
  'engine.apo.reinstall': '重新安装 Equalizer APO…',
  'engine.title': 'FluidEQ 该如何处理你的声音？',
  'engine.subtitle': '整台电脑只运行一个引擎。切换时 Windows 会请求一次授权。',
  'engine.now': '当前：{engine}',
  'engine.recommended': '推荐',
  'engine.fluid.name': 'FluidEQ 引擎',
  ...
  'prereq.install.apo': '安装 APO',
  'prereq.install.fluid': '安装 FluidEQ 引擎',
  'prereq.retry': '重试',
  'prereq.dismiss': '忽略',
  'prereq.credit.apo':
    'Equalizer APO 已随 {product} 一同提供，不会下载任何内容。它的安装程序会询问要均衡哪些音频设备，并在结束后要求重启。这是 {author} 的独立 GPLv2 项目，原样附带。',
  'prereq.starting': '正在启动…',
  'prereq.bundleMissing':
    '此版本缺少随附的 Equalizer APO。将改为打开其官方项目页面。',
  'prereq.notStarted':
    'Equalizer APO 没有启动——需要管理员权限。请重试并同意 Windows 的提示。',
  'whatsNew.eyebrow': '版本历史',
  'whatsNew.title': 'FluidEQ 更新日志',
  ...
  'sidebar.state.on': '{engine} · 已开启',
  'sidebar.state.off': '已关闭',
  'sidebar.state.notOnOutput': '未作用于此输出',
  'sidebar.preamp': '前级增益',
  'sidebar.autoLabel': '自动',
  ...
  'config.absent.fluid': 'FluidEQ 还没有写入 FluidEQ 引擎的配置。',
  'config.status.notIncluded':
    'Equalizer APO 没有包含这份配置，下面的内容全都没有生效。',
  'config.status.notIncluded.fluid':
    'FluidEQ 引擎没有读取这份配置，下面的内容全都没有生效。',
  'config.status.engineOff':
    '系统均衡已关闭——这份配置没有指定任何输出，所以 Equalizer APO 不会应用其中的任何内容。',
  'config.status.engineOff.fluid':
    '系统均衡已关闭——这份配置没有指定任何输出，所以 FluidEQ 引擎不会应用其中的任何内容。',
  'config.status.active': '已生效——Equalizer APO 正在应用这份配置。',
  'config.status.active.fluid': '已生效——FluidEQ 引擎正在应用这份配置。',
  'config.outputsAria': 'Equalizer APO 配置里的输出',
  'config.outputsAria.fluid': 'FluidEQ 引擎配置里的输出',
  ...
  'config.hint.saving.fluid': '保存会写入文件；FluidEQ 引擎随即读取。',
  'config.edit': '编辑',
  'config.cancel': '取消',
  'config.save': '保存',
