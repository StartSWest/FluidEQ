const terms = {
  ...
  'terms.membership.p4':
    '会员结束后，Plus 样式和会员创作的场景会重新锁定，FluidEQ 回到免费样式；你创作的内容不会被删除。没有网络连接时，Plus 仍可使用到已付费周期结束；如果应用未能确认续订，在此之后最多还可使用 {graceDays} 天。免费试用、赠送的 Plus 和通过发布赚得的月份，都在各自的日期结束：没有任何东西会续订它们，因此之后没有宽限期。免费的一切永远不受影响。',
  'terms.membership.p5':
  ...
  'terms.sent.membership.who':
    '只有你和作者。服务会确认你的会员资格，查找用你已确认的邮箱完成的 Buy Me a Coffee 付款，并读取你同意的是哪个版本的条款，以便条款变更时应用能通知你。同一个令牌还会询问这个账户是否可以开始免费试用，以及它赚得的月份情况。',
  ...
  'terms.fair.p1':
    '积分来自收听和点赞：每播放一小时音乐得 {hourPoints} 分，每个播放音乐至少 {activeMinutes} 分钟的日子得 {dayPoints} 分，你的场景每获得一个赞得 {likePoints} 分。排行榜显示前 100 名，并且只在账户拥有付费、获赠或通过发布赚得的 Plus 期间对其排名：免费试用不参与排名。无论哪种情况，其天数都会保留，Plus 恢复后重新计入。',
  'terms.fair.p2':
  ...
  'terms.keep.p4':
    '你的账户：申请删除后，它会在 {deletionDays} 天内被删除，一并删除的还有你的个人资料、排行榜上的每日记录、会员资格、免费试用和赚得的月份、同意记录、点赞、添加、举报，以及你发布的场景及其文件。赠送给你邮箱的 Plus 会一直保留，直到作者将其移除。',
  'terms.keep.p5':
