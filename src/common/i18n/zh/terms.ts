const terms = {
  ...
  'terms.fair.p1':
    '收听时长由你电脑上的应用统计，服务器无法亲眼看到。因此它会核查每个数字：每天不超过 {capHours} 小时，不接受尚未开始的日期，不接受超过 {windowDays} 天的旧数据，任何一天的增长都不能快过时钟。你各台电脑的数字会加总为同一天，而这一天的增长同样不能快过时钟，因此多台电脑同时播放，加起来也不会超过实际经过的时间。消息和提及在服务器上根据实际发布的内容统计。',
  'terms.fair.p2':
