const terms = {
  ...
  'terms.membership.p1':
    '使用免费账户，你可以浏览“可视化”，查看每个已发布场景的图片和详情，试用 FluidEQ 的每个免费示例场景各 {tasteSeconds} 秒，并查看排行榜。Plus 可以播放和添加每一个场景，解锁 Plus 外观，让你在工作室中创作场景并导出或发布，让你加入排行榜，还能把场景放到你的桌面和 RGB 灯光上。价格为 {price}，在每个已付费周期结束时自动续订，直到你取消。 Plus 也可以不用付费就获得。当 FluidEQ 提供免费体验时，在该活动开始当天或之后创建的账户可以使用一次，为期 {trialDays} 天，无需银行卡，结束时也不会扣费。你发布并通过审核员批准的场景，可以换来一个月的 Plus。这两者都不是订阅：它们不会续订，也永远不会为此扣费。一旦你的某个场景已为你赢得一个月，即使没有 Plus，工作室也会为你保留一个可用的项目，让你可以发布下一个场景；导出文件仍然需要 Plus。',
  'terms.membership.p2':
    '付款由 Buy Me a Coffee 按其自身条款处理。FluidEQ 从不接触你的银行卡或银行信息。你可以随时在 Buy Me a Coffee 上取消：Plus 会保持到已付费周期结束，之后不再扣费。',
  'terms.membership.p4':
    '会员结束后，Plus 外观和会员创作的场景会重新锁定，FluidEQ 回到免费样式；你创作的内容不会被删除。没有网络连接时，Plus 仍可使用到已付费周期结束；如果应用未能确认续订，在此之后最多还可使用 {graceDays} 天。免费体验、赠送的 Plus 和通过发布赚得的月份，都在各自的日期结束：没有任何东西会续订它们，因此之后没有宽限期。免费的一切永远不受影响。',
  'terms.membership.p5':
  ...
  'terms.sent.membership.who':
    '只有你和作者。服务会确认你的会员资格，查找用你已确认的邮箱完成的 Buy Me a Coffee 付款，并读取你同意的是哪个版本的条款，以便条款变更时应用能通知你。同一个令牌还会询问这个账户是否可以开始免费体验，以及它赚得的月份情况。',
  ...
  'terms.sent.looks.who':
    '不会保存任何内容。每个外观都带有签名，你的电脑会在播放前校验签名。',
  'terms.sent.catalogue.what': '不含任何关于你的信息',
  'terms.sent.catalogue.when':
    'FluidEQ 启动时（最多每几个小时一次），用于显示有哪些 Plus 外观，无论是否有账户',
  'terms.sent.catalogue.who': '不会保存任何内容。该请求只获取公开的外观列表。',
  ...
  'terms.fair.p1':
    '积分来自收听和点赞：每播放一小时音乐得 {hourPoints} 分，每个播放音乐至少 {activeMinutes} 分钟的日子得 {dayPoints} 分，你的场景每获得一个赞得 {likePoints} 分。排行榜显示前 100 名，并且只在账户拥有付费、获赠或通过发布赚得的 Plus 期间对其排名：免费体验不参与排名。无论哪种情况，其天数都会保留，Plus 恢复后重新计入。',
  'terms.fair.p2':
  ...
  'terms.rules.p4':
    '为了让服务对每个人都正常运行，每个账户每小时最多可导出或发布 {sharesPerHour} 个场景（被拒绝的尝试也计算在内），并且最多可保持 {maxPublished} 个场景处于已发布状态。你发布的场景及其每个新版本，都要等 FluidEQ 的作者读过之后，别人才能看到；一个自然月内可以送审的数量有上限，达到时工作室会告诉你。',
  ...
  'terms.keep.p4':
    '你的账户：申请删除后，它会在 {deletionDays} 天内被删除，一并删除的还有你的个人资料、排行榜上的每日记录、会员资格、免费体验和赚得的月份、同意记录、点赞、添加、举报，以及你发布的场景及其文件。赠送给你邮箱的 Plus 会一直保留，直到作者将其移除。',
  'terms.keep.p5':
  ...
  'terms.keep.p6':
    '在你的电脑上：Plus 外观和你添加的场景（加密保存）；图库图片（最多 128 MB）；以及被封禁场景的列表。它们会一直保留，直到你将其移除或卸载 FluidEQ。',

  'terms.looks.title': 'Plus 外观',
  'terms.looks.p1':
    'Plus 外观是作者本人的作品，在你作为会员期间授权你个人使用。请不要复制、分享或转售。',
  'terms.looks.p2':
  ...
  'terms.scenes.p3':
    '导出场景时，你允许 FluidEQ 检查它（包括与 FluidEQ 自己的场景进行比对）、移除其代码中的注释，并以你的显示名称和账户 ID 为它签名，让其他 Plus 会员可以播放它，并看到它是你创作的。这就是全部的许可：FluidEQ 的作者不会出售你的场景或将其用于广告，未经你事先同意不会把它变成 Plus 外观之一，也不会阻止你对自己的作品做任何其他事情。',
  'terms.scenes.p4':
    '分享是 Plus 的一部分，而不是一份工作：没有人因场景拿到钱，也没有人为场景付钱。你得到的回报，是其他会员分享的所有场景；当 FluidEQ 的作者批准你的场景时，还会得到一个月的 Plus——那是使用权，绝不是现金。',
  'terms.scenes.p5':
    '如果你加入了排行榜，为你的场景点赞的会员会让你获得积分。点赞由服务器统计；参见“排行榜的公平规则”。',
  'terms.scenes.p6':
  ...
  'terms.scenes.p9':
    '如果你把场景发布到“可视化”，你也允许 FluidEQ 的作者在别人看到之前先阅读它，允许 FluidEQ 在你取消发布之前把它保存在那里，把它的图片、名称、分类和版本说明连同你的显示名称和用户名展示给所有已登录 FluidEQ 的人，并把场景本身提供给 Plus 会员播放和添加。你为它设置的氛围元素会随场景一起分发，选择“氛围”模式的会员会在自己的窗口周围看到这些元素。无论是否拥有 Plus，你都可以随时取消发布。',
  'terms.scenes.p10':
  ...
  'terms.elsewhere.p9':
    '你的 AI 助手：只有在工作室中打开“让你的 AI 看到舞台”时（复制给 AI 的提示词也会将其打开，除非你自己关掉过它），这台电脑上持有你密钥的 AI 工具才能向 FluidEQ 请求你工作室场景的图片（场景无法播放时附上 FluidEQ 给出的原因），以及描述你所播放音乐如何变化的数字——绝不包括声音本身，也绝不透露是哪首歌。该工具如何使用这些内容，受其自身条款约束。',
  'terms.elsewhere.p4':
