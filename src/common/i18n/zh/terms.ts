const terms = {
  ...
  'terms.membership.p1':
    '使用免费账户，你可以浏览“可视化”，查看每个已发布场景的图片和详情，试用 FluidEQ 的每个免费示例场景各 {tasteSeconds} 秒，并查看排行榜。Plus 可以播放和添加每一个场景，解锁 Plus 样式，让你在工作室中创作场景并导出或发布，让你加入排行榜，还能把场景放到你的桌面和 RGB 灯光上。价格为 {price}，在每个已付费周期结束时自动续订，直到你取消。 Plus 也可以不用付费就获得。当 FluidEQ 提供免费试用时，在该活动开始当天或之后创建的账户可以使用一次，为期 {trialDays} 天，无需银行卡，结束时也不会扣费。你发布并通过审核员批准的场景，可以换来一个月的 Plus。这两者都不是订阅：它们不会续订，也永远不会为此扣费。',
  'terms.membership.p2':
  ...
  'terms.sent.scenePublish.who':
    '在审核员批准之后，在你取消发布之前，所有已登录 FluidEQ 的人都能在“可视化”中看到它的图片、名称、分类、版本说明、点赞数和添加数，以及你的显示名称、用户名和作者主页。只有 Plus 会员可以播放和添加这个场景。FluidEQ 的作者会保存该场景，并像导出时一样保存你发布它的记录。',
