...
const tour: Partial<Dictionary> = {
  ...
  'tour.engine.how':
    'インストール時に FluidEQ エンジンを選ぶか、「オーディオ操作」メニューを開いてそこで選びます。次に「DSP」を開き、いずれかのアプリで音を再生しながらステージをオンにします。',
  ...
  'tour.plus.lead':
    'FluidEQ の成長を支える任意のメンバーシップです。専用の新しいタブには、グラフィックスカードで描くシーン、自分のシーンを作れるスタジオ、ランキング、デスクトップの背景、ダイナミック ライティングがそろっています。無料だったものはすべて無料のままです。',
  'tour.plus.point1':
  ...
  'tour.desktop.point2':
    '全画面アプリの使用中、PC のロック中、バッテリー駆動中は一時停止します。',
  ...
  'tour.share.step2':
    'その PC で FluidEQ を開き、「音声を共有」で「このコンピューターの音声を送信」を選び、「音楽」か「ゲーム/動画」を指定して、コードを貼り付けてから「接続して送信」を押します。システム音声が流れ始めます。',
  'tour.share.step3Title': '聴きながら音量を調整',
  'tour.share.step3':
    '「音楽」は途切れない再生のために大きめの安全バッファを保ち、「ゲーム/動画」はリップシンクのために最小遅延で動きます。各送信側の音は受信側の出力にミックスされ、その EQ で整えられ、「音量」で大きさが決まります。受信側の再生バーには最新の送信元の曲が表示され、そのボタンはネットワーク越しに動作します。',
  ...
  'tour.dsp.lead':
    'スタジオ級のステージを順に並べたラックです: ノーマライザー、ノイズ除去、エキサイター、ベースフォージ、イコライザー、ベースパンチ、ディメンション、マキシマイザー、マスター。さらにライブラリの曲間のクロスフェード。FluidEQ エンジンではコンピューターで再生されるすべての音に、Equalizer APO ではライブラリにかかります。各ステージはライブグラフとプリセット、そのステージの効果だけを聴く「アイソレート」ボタンを持つカードです。',
  'tour.dsp.point1':
    'ノイズ除去は再生しながらヒス、ハム、クリックを修復し、ニューラル音声クリーナーはライブラリの曲で働きます。',
  'tour.dsp.point2':
    'ベースフォージはベースの下に本物の 1 オクターブを足し、ベースパンチはアタック、サステイン、ブルームを整えます。「ミックス」は最大 200% です。',
  'tour.dsp.point3':
  ...
  'tour.output.title': 'セカンド出力のプロファイル',
  'tour.output.subtitle':
    'ヘッドセットとスピーカーを同時に、それぞれのプロファイルで',
  'tour.output.lead':
    'ヘッドセットとスピーカーを別々のEQで同時に鳴らせます。セカンド出力にはメイン出力のEQ適用前の音声が届き、専用の保存済みプロファイルが適用されます。ルーティングドライバーは不要です。',
  'tour.output.point1':
    'セカンド出力で別のデバイスをオンにし、その音量を調整します。',
  'tour.output.point2':
  ...
  'tour.output.how':
    'EQタブの右側でセカンド出力を開きます。デバイスをオンにして名前の下のEQプロファイルを選び、音量とゲーム/動画または音楽を設定します。',
  'tour.output.open': 'EQ を開く',
  'tour.output.imageAlt':
    'BlackShark V2 Proが有効なセカンド出力パネル。EQプロファイル選択欄、音量スライダー、ゲーム/動画と音楽のモードが表示されています。',
