...
/**
 * Continuous mode: nudging the smart EQ while the music plays.
 *
 * The upper half of what was one file. The layer below decides what a
 * correction *is* — which bands, at what frequencies, clamped how. This decides
 * when to move one and by how much, which is a different question with its own
 * vocabulary: a deadband, a trigger, a settle, a memory, a reset.
 *
 * The dependency runs one way and exactly one function crosses it. That is what
 * makes this a layer rather than a split: continuous mode needs the layer to
 * sanitise what it produces, and the layer needs nothing from here.
 */
/**
