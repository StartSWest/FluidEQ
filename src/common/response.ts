...
/**
 * How much louder this chain actually makes typical music, in dB.
 *
 * The headroom question asked a different way, and the difference is the width
 * of the boost. `getChainPeakGain` answers "how high does the chain go anywhere"
 * — a worst case, and the right answer if you assume full-scale content sitting
 * exactly on the highest point. This answers "how much more energy comes out
 * than went in", which is what the level actually does.
 *
 * The gap between them is large and it is paid continuously. A narrow +12 dB
 * band spans a fraction of an octave out of ten, so it adds almost nothing to
 * the total: the peak metric reserves twelve decibels for it and drops
 * everything by twelve, all evening, to protect against a moment that needs
 * full-scale content at that exact frequency. A broad shelf of the same height
 * genuinely does raise the whole output and is charged accordingly, which is the
 * behaviour worth keeping.
 *
 * Summed in the power domain, because energies add and decibels do not, and
 * weighted by `PROGRAM_ALLOWANCE` — a range where music has little to begin with
 * contributes little however hard it is lifted. Over log-spaced samples, so an
 * octave counts the same wherever it is; that is also what makes a narrow filter
 * cheap and a wide one expensive, which is the entire point.
 *
 * THE HONEST COST: this can clip where the peak metric would not. A narrow lift
 * low down, where music does reach full scale, is exactly the case it charges
 * least for and exactly where a clip is most likely. It is a deliberate trade of
 * a rare, audible, detectable event against a permanent loss of level on every
 * second of every track — and the live capture already watches for railed
 * samples, so the rare case is observable rather than silent.
 */
export const getChainLoudnessGain = (
  filters: Array<Pick<IFilter, 'type' | 'frequency' | 'gain' | 'quality'>>,
): number => {
  if (filters.length === 0) {
    return 0;
  ...
  SAMPLE_FREQUENCIES.forEach((frequency) => {
    ...
    // How much of the programme's energy lives here, from the same model the
    // peak metric uses: the allowance is the shortfall below the busiest region,
    // so its negative is the relative level.
    const share = 10 ** (-getProgramAllowance(frequency) / 10);
    weighted += share * 10 ** (total / 10);
    weight += share;
  });
  ...
  return Math.round(10 * Math.log10(weighted / weight) * 100) / 100;
};
