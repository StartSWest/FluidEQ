...
/**
 * An inherited start is only a suggestion. Early Enter presses explicitly set
 * the real start; once enough of the phrase has elapsed, Enter may use the
 * suggestion as the start and capture the end in one action.
 */
export const karaokeMakerSuggestedStartIntent = (
  suggestedStartMs: number,
  playheadMs: number,
  estimatedSpanMs: number,
): TKaraokeMakerLineCaptureIntent => {
  if (
    !Number.isFinite(suggestedStartMs) ||
    !Number.isFinite(playheadMs) ||
    !Number.isFinite(estimatedSpanMs)
  ) {
    return 'start';
  }
  const endThresholdMs = Math.max(
    600,
    Math.min(1_800, Math.max(1_000, estimatedSpanMs) * 0.45),
  );
  return playheadMs - suggestedStartMs >= endThresholdMs ? 'end' : 'start';
};
...
/**
 * Moves one complete lyric line from any token inside it. The editor uses the
 * first word as the line handle, while this model helper keeps every word and
 * linked melody note at the same relative distance. Neighbouring lyric lines
 * are hard boundaries, so a manual sentence adjustment cannot create a new
 * overlap.
 */
export const shiftKaraokeMakerLineFromToken = (
  project: IKaraokeMakerProject,
  tokenId: string,
  requestedDeltaMs: number,
): IKaraokeMakerProject => {
  ...
  const linkedNotes = project.melody.notes.filter(
    (note) => note.tokenId !== undefined && tokenIds.has(note.tokenId),
  );
  const lineStartMs = Math.min(
    ...timedTokens.map((token) => token.startMs as number),
    ...linkedNotes.map((note) => note.startMs),
  );
  const lineEndMs = Math.max(
    ...timedTokens.map((token) => token.endMs as number),
    ...linkedNotes.map((note) => note.endMs),
  );
  const precedingTokens = project.lyrics.lines
    .slice(0, lineIndex)
    .filter((candidate) => !karaokeMakerLineIsSection(candidate))
    .flatMap((candidate) => candidate.tokens);
  const followingTokens = project.lyrics.lines
    .slice(lineIndex + 1)
    .filter((candidate) => !karaokeMakerLineIsSection(candidate))
    .flatMap((candidate) => candidate.tokens);
  const previousEndMs = precedingTokens.reduce(
    (latest, token) => Math.max(latest, token.endMs ?? token.startMs ?? 0),
    0,
  );
  const nextStartMs = followingTokens.reduce(
    (earliest, token) =>
      token.startMs === undefined
        ? earliest
  ...
  const lowerDeltaMs = previousEndMs - lineStartMs;
  let upperDeltaMs = Number.POSITIVE_INFINITY;
  if (Number.isFinite(nextStartMs)) {
    upperDeltaMs = nextStartMs - lineEndMs;
  } else if (project.audio.durationMs !== undefined) {
    upperDeltaMs = Math.max(0, project.audio.durationMs) - lineEndMs;
  }
  const roundedRequest = Math.round(requestedDeltaMs);
  const deltaMs =
    lowerDeltaMs <= upperDeltaMs
      ? Math.max(lowerDeltaMs, Math.min(upperDeltaMs, roundedRequest))
      : Math.max(-lineStartMs, roundedRequest);
  if (deltaMs === 0) {
    return project;
  }
          ...
          : {
              ...
              startMs:
                candidate.startMs === undefined
                  ? undefined
                  : candidate.startMs + deltaMs,
              endMs:
                candidate.endMs === undefined
                  ? undefined
                  : candidate.endMs + deltaMs,
