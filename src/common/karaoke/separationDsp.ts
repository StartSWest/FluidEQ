 ...
 *
 * Iterative rather than recursive because this runs 2,200 times per chunk and
 * the call overhead is measurable. `inverse` selects the sign of the twiddle
 * factor only — the caller applies the 1/N scaling, so a forward transform
 * followed by an inverse one is the identity up to that factor.
 */
