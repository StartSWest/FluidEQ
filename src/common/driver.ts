...
export const DRIVER_PROFILES: IDriverProfile[] = [
  {
    ...
    note: 'Only the bass is corrected here. A treble filter was considered and left out: diaphragm breakup is set by dome geometry and tension rather than by driver size, so two 40 mm drivers can break up an octave apart — anywhere from about 5 to 12 kHz. Any frequency picked for you would be a guess, and a guess in the wrong place in the treble is worse than nothing.',
  },
  {
    id: 'planar-headphone',
    ...
    category: 'headphone',
    filters: [],
    note: 'This one deliberately applies nothing. Planars have no bass shortfall to fill and no sharp breakup peak to notch — the topology removes both. A top-octave trim was tested and rejected: where a large diaphragm interferes with itself scales with its size, so the same shelf lands near 9-11 kHz on one brand and near 6-7 kHz on another, which would make it backwards for roughly half of all planar owners. If your planar tires you up top, use your own bands where you can hear what you are changing.',
  },
  {
    id: 'dynamic-iem',
    name: 'Single dynamic',
    ...
    category: 'iem',
    filters: [],
    note: 'Nothing applied on purpose. The peak people want cut here is the ear canal resonance, not the driver, and it moves by most of an octave between people, tips and how deep you push them in. Change tips before reaching for EQ. If your model is in the AutoEQ or Squiglink databases, use that — for this type especially, a measurement beats any family guess by a wide margin.',
  },
  {
