 ...
 * block and no `Include:`. Written with the power switch off, into the engine
 * being left at a switch, and into the engine in use when FluidEQ quits — at
 * the next launch the startup read finds no block in it to adopt, so the saved
 * EQ is what gets written back.
 */
export const DISABLED_ROOT_TEXT = [
