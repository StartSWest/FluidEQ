...
/**
 * DC out of the presence generator, which is not optional.
 *
 * `feq_harmonic_sample`'s even order is `2x^2` without Chebyshev's -1 — see
 * `harmonics.h` for the tail that constant left behind — and the mean of
 * `2x^2` is `2 * mean(x^2)`, which is positive for everything that is not
 * silence. So the generator hands back an offset the size of the harmonic it
 * came with, into the one band with no headroom to spare. The Exciter blocks
 * the same DC for the same reason.
 *
 ...
 */
constexpr double kPresenceDcHz = 20.0;
 ...
 * a control, and a split corner of zero is a filter that returns NaN forever.
 */
...
void feq_bass_forge_init(FeqBassForge* state, float* low, float* scratch) {
  ...
  // A negative `mix` is the sentinel for "no block has run yet". Without it
  // every session starts with the smoothers at zero and fades the user's own
  // settings in over the first 18 ms, which reads as the stage arriving late.
  state->drive = 1.0;
...
void feq_bass_forge_reset(FeqBassForge* state) {
  ...
  // Shut rather than open: nothing has been measured yet, and a divider that
  // runs before its level is known is a burst of octave at every seek.
...
void feq_bass_forge_process(FeqBassForge* state, float* const* channels,
  ...
  // Two channels of low band is what the buffers hold, so a surround block
  // gets its front pair forged and the rest passed through untouched.
  ...
  /**
   * One stage here and two above.
   *
   * The octave of a 50 Hz note lands exactly on this corner, and a 4th-order
   * one would take 6 dB off the thing the generator exists to make. A single
   * Butterworth stage is still 12 dB down at 12 Hz, which is what the filter
   * is actually for: an amplifier will try to reproduce infrasound, and a
   * woofer will move a long way doing it.
   */
  ...
  const double dc_pole = std::exp((-2.0 * kPi * kPresenceDcHz) / sample_rate);

  const double target_drive =
      std::pow(10.0, clamp(settings->drive_db, 0.0, kMaxDriveDb) / 20.0);
  ...
  // output below is written as `input + (forged band - dry band)`, and it is
   ...
   * band.
   *
   * `|x|` rather than a smoothed envelope, which is the same shape a BOSS OC-2
   * makes and is better here for two reasons. A follower fast enough to keep
   * up with a bass note ripples at twice the note, and that ripple multiplied
   * by the square is amplitude modulation with sidebands the lowpass below
   * cannot reach. And `|x|` carries the note's own dynamics into the octave,
   * where a follower would flatten them into a square of constant height.
   *
   * It is monophonic and it tracks the loudest partial of a chord. That is a
   * property of every analogue divider ever sold and is exactly why
   * `sub_amount` blends rather than replaces.
   *
   * The crossings are read before Drive rather than after, because a positive
   * gain cannot move one. Drive is applied where it does something, which is
   * the level the floor below is compared against.
   */
  ...
  for (uint32_t at = 0; at < frames; ++at) {
    ...
    /**
     * What Drive can and cannot do, which is not what it does in a distortion.
     *
     * Both generators are level-normalised — the divider's output is matched
     * back onto the band it came from, and `feq_harmonic_sample` divides the
     * band's level out before it shapes anything — so a gain in front of
     * either leaves the result the same size. What it does move is where the
     * programme sits against the floor below: at 0 dB the octave appears only
     * under notes that are genuinely loud, and at 12 dB it follows the quiet
     * ones down as well. That is the whole of it, and it is the honest half of
     * what a "hot" control usually claims to be.
     */
    ...
    // A ramp rather than a threshold, fully open at twice the floor: the
    // divider fades in under a note instead of switching on inside one.
    ...
    // seeing the divider run, or it would go stale every time the gate shut
    // and arrive wrong on the next note.
    ...
    // one here would be a second copy of the band under the band.
    const double shaped =
        feq_harmonic_sample(&state->harmonic, mono, state->presence_amount,
                            state->texture, sample_rate);
    state->dc_output = shaped - state->dc_input + dc_pole * state->dc_output;
    state->dc_input = shaped;

    const double added =
        (sub * state->sub_amount + state->dc_output) * state->mix;
    ...
    /**
     * The no-free-loudness rule, and the reason it is a gain on the whole band
     * rather than on what was added to it.
     *
     * Scaling the addition holds at full mix and nowhere else. Scaling the band
     * holds everywhere, and it makes a mix of zero exact rather than close: the
     * two mean squares below are then fed identical numbers, so the ratio is
     * one to the last bit and the band comes back through this arithmetic as
     * itself.
     */
    ...
    forged /= static_cast<double>(used);
